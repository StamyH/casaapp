...
describe('calcolaQuote', () => {
  ...
  test('percentuale — 0% al pagatore', () => {
    const quote = calcolaQuote(100, 'Federico', 'Riccardo', 'percentuale', 0);
    expect(quote['Federico']).toBe(0);
    expect(quote['Riccardo']).toBe(100);
  });

  test('divisione sconosciuta — fallback a metà', () => {
    const quote = calcolaQuote(100, 'Riccardo', 'Federico', 'sconosciuta');
    expect(quote['Riccardo']).toBe(50);
    expect(quote['Federico']).toBe(50);
  });
});
...
describe('calcolaBilancio', () => {
  ...
  test('due spese uguali da pagatori diversi — in pari', () => {
    const spese = [
      { importo: 100, pagatore: 'Riccardo', altroUtente: 'Federico', divisione: 'metà', percentuale: 50 },
      { importo: 100, pagatore: 'Federico', altroUtente: 'Riccardo', divisione: 'metà', percentuale: 50 },
    ];
    const bilancio = calcolaBilancio(spese, UTENTI);
  ...
  test('spesa tutto_altro — debitore è chi ha pagato', () => {
    const spese = [
      { importo: 80, pagatore: 'Riccardo', altroUtente: 'Federico', divisione: 'tutto_altro', percentuale: 100 }
    ];
    const bilancio = calcolaBilancio(spese, UTENTI);
    expect(bilancio.debitore).toBe('Federico');
  ...
  test('compensazione incrociata — debiti si sottraggono', () => {
    const spese = [
      { importo: 100, pagatore: 'Riccardo', altroUtente: 'Federico', divisione: 'metà', percentuale: 50 },
      { importo: 40, pagatore: 'Federico', altroUtente: 'Riccardo', divisione: 'metà', percentuale: 50 },
    ];
